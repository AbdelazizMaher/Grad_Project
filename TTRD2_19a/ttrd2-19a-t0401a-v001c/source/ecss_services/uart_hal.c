...
void HAL_uart_tx(TC_TM_app_id app_id, uint8_t *buf, uint16_t size)
{
    ...
    if(app_id == EPS_APP_ID) {
    	MYDMA_Init(DMA1_Stream3, DMA_Channel_4, (uint32_t)&(USART3->DR), (uint32_t)buf, size);
    	DMA_ClearFlag(DMA1_Stream3,DMA_FLAG_TCIF7);
			MYDMA_Enable(USART3,DMA1_Stream3, size);
			//EPS_U1_UART_send(buf, size);
			UART2_BUF_O_Write_String_To_Buffer("msg EPS SENT\n");
...
//			COMMS_U4_UART_send(buf, size);
			UART2_BUF_O_Write_String_To_Buffer("msg COMMS SENT\n");
			UART2_BUF_O_Send_All_Data();			
    }
...
//    	DMA_ClearFlag(DMA2_Stream7,DMA_FLAG_TCIF7);
//			MYDMA_Enable(USART6,DMA2_Stream7, size);
			UART2_BUF_O_Write_String_To_Buffer("msg ADCS SENT\n");
			UART2_BUF_O_Send_All_Data();			
    }	
