...
void EPS_U1_BUF_O_Init(uint32_t BAUD_RATE)
{
  ...
  TIMEOUT_T3_USEC_Init();
	...
	// USART1 clock enable 
	RCC_APB1PeriphClockCmd(EPS_U1_UART_RCC, ENABLE);
	...
	for ( ;len > 0;len--, ++d)
	{
		...
		if(*d == 0x7E)
		{
			if(EPS_U1_FLAGS.Error_Flag)
			{
				...
				UART2_BUF_O_Write_String_To_Buffer("msg EPS ERRR\n");
			...
			{
				UART2_BUF_O_Write_String_To_Buffer("Hello EPS 1\n");
				UART2_BUF_O_Send_All_Data();
				if((EPS_U1_FLAGS.End1_Flag == 0) && (EPS_U1_FLAGS.Start1_Flag ==0))
				{
					UART2_BUF_O_Write_String_To_Buffer("Hello EPS 2\n");
				...
				else if((EPS_U1_FLAGS.End1_Flag == 0) && (EPS_U1_FLAGS.Start1_Flag ==1))
				{
					...
					EPS_U1_DMA_flag =1;
					UART2_BUF_O_Write_String_To_Buffer("msg EPS rcvd1\n");
				}
				else
				{
			...
			else if(EPS_U1_FLAGS.ID_Flag == 2)
			{
				...
				else if((EPS_U1_FLAGS.End2_Flag == 0) && (EPS_U1_FLAGS.Start2_Flag ==1))
				{
					...
					EPS_U1_DMA_flag =1;
					UART2_BUF_O_Write_String_To_Buffer("msg EPS rcvd2\n");
				}
				else
				{
			...
			else if(EPS_U1_FLAGS.ID_Flag == 3)
			{
				...
				else if((EPS_U1_FLAGS.End3_Flag == 0) && (EPS_U1_FLAGS.Start3_Flag ==1))
				{
					...
					EPS_U1_FLAGS.ID_Flag = 1;
					EPS_U1_DMA_flag =1;
					UART2_BUF_O_Write_String_To_Buffer("msg EPS rcvd3\n");
				}
				else
				{
