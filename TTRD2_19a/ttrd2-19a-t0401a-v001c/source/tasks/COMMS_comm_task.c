...
#define COMMS_U4_SettingDelay 60
extern uint8_t COMMS_U4_Sent_Flag;
static uint8_t	COMMS_U4_Ready=1;
uint8_t COMMS_U4_SET_Counter=0;
...
void COMMS_U4_BUF_O_Init(uint32_t BAUD_RATE)
{
  ...
  // Set pin init 
	...
	hdma_usart6_rx.DMA_Channel = COMMS_U4_DMA_Channel;
	hdma_usart6_rx.DMA_PeripheralBaseAddr = (uint32_t) (&(COMMS_U4_UARTX->DR));; 
	hdma_usart6_rx.DMA_Memory0BaseAddr = (uint32_t)COMMS_U4_Rx_buffer_g;  //
	hdma_usart6_rx.DMA_DIR = DMA_DIR_PeripheralToMemory;
	hdma_usart6_rx.DMA_BufferSize = COMMS_U4_RX_BUFFER_SIZE_BYTES;
	...
	DMA_Init(COMMS_U4_DMA_RX_STREAM, &hdma_usart6_rx);
...
//	hdma_usart6_tx.DMA_Channel = COMMS_U4_DMA_Channel;
//	hdma_usart6_tx.DMA_PeripheralBaseAddr = (uint32_t) (&(COMMS_U4_UARTX->DR)); 
//	hdma_usart6_tx.DMA_Memory0BaseAddr = (uint32_t)COMMS_U4_Rx_buffer_g;
//	hdma_usart6_tx.DMA_DIR = DMA_DIR_MemoryToPeripheral;
//	hdma_usart6_tx.DMA_BufferSize = COMMS_U4_TX_BUFFER_SIZE_BYTES+1;
//	hdma_usart6_tx.DMA_Mode = DMA_Mode_Normal;
//	DMA_Init(COMMS_U4_DMA_TX_STREAM, &hdma_usart6_tx);
...
void COMMS_U4_DMA_CHECK(struct uart_data *uart_data)
{
	...
	//UART2_BUF_O_Send_All_Data();			/* Current position is over previous one */
...
SAT_returnState COMMS_U4_UART_recv(struct uart_data *uart_data) 
{
	if(COMMS_U4_DMA_flag)
	{
		...
		UART2_BUF_O_Write_String_To_Buffer("\n");
		if(length > MAX_PKT_SIZE) // here need to make a request for the corrupted packet
		{
			UART2_BUF_O_Write_String_To_Buffer("\nSize Error:");
			UART2_BUF_O_Write_String_To_Buffer("\n--> Size:");
		...
		UART2_BUF_O_Write_String_To_Buffer("COMMS_U4_UART_recv\n");
