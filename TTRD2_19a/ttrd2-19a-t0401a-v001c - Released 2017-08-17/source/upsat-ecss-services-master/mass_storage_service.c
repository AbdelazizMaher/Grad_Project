...
extern SAT_returnState power_control_api(FM_dev_id did, FM_fun_id fid, uint8_t *state);
...
SAT_returnState mass_storage_app(tc_tm_pkt *pkt) {
    ...
    if(pkt->ser_subtype == TC_MS_DISABLE) {
			...
			{
				power_control_api(OBC_SD_DEV_ID, P_OFF, NULL);
				mass_app++;
			}        
        ...
        if( mass_app == 2000 )
				...
				{
					power_control_api(OBC_SD_DEV_ID, P_ON, NULL);
				}        
        //HAL_sys_delay(1);
            ...
            if(sid == SRAM) {
              res = sram_hard_delete();
                ...
                for(uint8_t i = 0; i < MAX_F_RETRIES; i++) {
                  ...
                  if((res = mass_storage_hard_delete(sid)) != SATRF_LOCKED)
                  {
										mass_storeFLAG = 0;
										break; 
									}
									else if( mass_app == MAX_F_RETRIES)
									{
										mass_app=0;
										mass_storeFLAG = 0;
										break;
